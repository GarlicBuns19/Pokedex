  ...
  let data = await fetch(url)
    ...
    .then((data) => {
      ...
      data.results.forEach((n) => {
        btnsearch.innerHTML += `<button id="pokemon" onclick="pokemonUrl('${
          n.url
        }')">${n.name.charAt(0).toUpperCase() + n.name.substring(1)}
        </button>`;
    ...
    .then((pokemon) => {
      ...
      pokemon.types.forEach((typeName) => {
        document.getElementById("poke-Type").innerHTML += `
                <p class="card-text">
                    ...
                    <img src="./images/pokemon-types/${
                      typeName.type.name.charAt(0).toUpperCase() +
                      typeName.type.name.substring(1)
                    } type.ico" alt="poke-Type-icon" id="typeNameImg">
                </p>
                `;
      });
      ...
      pokemon.abilities.forEach((abilityName) => {
        document.getElementById("poke-Ability").innerHTML += `
                <p class="card-text">
                    ${
                      abilityName.ability.name.charAt(0).toUpperCase() +
                      abilityName.ability.name.substring(1)
                    }
                </p>
                `;
      });
      ...
      document.querySelector(
        "#poke-Img"
      ).innerHTML += `<img src="${pokemon.sprites.other["official-artwork"].front_default}" class="card-img-top img-fluid" alt="Pokemon Pic">`;
...
footer.innerHTML = `<p>&copy Ra'ees Benny ${year} copyright</p>`
// =======================================================================================================================
// .then((p) => {
//     console.log(p.name)
